$(document).ready(function () {
    $("#pickup").autocomplete({
      source: function (e, t) {
        $.ajax({
          type: "GET",
          url: "https://booking.londontaxis247.co.uk/Home/Indextwo",
          dataType: "json",
          data: { Prefix: e.term },
          success: function (e) {
            var a = e.list;
        ...
        source: function (request, response) {
          ...
          $.ajax({
            ...
            success: function (data) {
              ...
              let combinedResults = [
                ...new Set([...validAddresses, ...liveResults]),
              ];
              let filteredResults = combinedResults.filter((addr) =>
                addr.toLowerCase().includes(request.term.toLowerCase())
              );
  ...
  $("#updateVia").click(function () {
    ...
    tempFields.forEach((field, index) => {
      ...
      if (trimmedField === "") {
        ...
        !validAddresses.some(
          (addr) => addr.toLowerCase() === trimmedField.toLowerCase()
        )
      ) {
          ...
          $(`#${myidd}`).html(`
                            ...
                            <div class="input-group-addon">
                                <button type="button" class="btn input-grp-btns" onclick="removeitem(this)">
                                    <img class="form-icons" src="images/delete.jpg" alt="luggage delete" width="20">
                                </button>
                            </div>`);
        } else {
          tempacc.push(item);
  ...
  $(document).ready(function () {
    ...
    $("#get-quotes").click(function () {
      ...
      //-------------------ajax call--------------
      var office_name = "LNT";
      var color_code = "f7b731";
      var office_details =
        "LNT,Heathrow Minicabs Cars, https://heathrowminicabscars.co.uk/, 02038131432";
      ...
      if (
        ...
        // window.location.href = "https://booking.londontaxis247.co.uk/OurVehicle/OurVehicle?luggage_text=" + inputsvalues + "&pickup=" + pickup + "&checkurl=" + true + "&dropoff=" + dropoff + "&office_details=" + office_details + "&luggageobject=" + obj + "&listviasaddress=" + listvias +"&tripFlag=" +TripFlag +"&mints="+WaitingMints;
  
        // window.location.href = "https://booking.londontaxis247.co.uk/OurVehicle/OurVehicle?luggage_text=" + inputsvalues + "&pickup=" + pickup + "&checkurl=" + true + "&dropoff=" + dropoff + "&office_details=" + office_details + "&luggageobject=" + obj + "&listviasaddress=" + listvias + "&tripFlag=" + TripFlag + "&mints=" + WaitingMints + "&fromDoorNumber=" + frmDrNmbr + "&toDoorNumber=" + toDrNmbr;
        ...
        window.location.href =
          "https://booking.londontaxis247.co.uk/OurVehicle/OurVehicle?luggage_text=" +
          inputsvalues +
          "&pickup=" +
          pickup +
          "&checkurl=" +
